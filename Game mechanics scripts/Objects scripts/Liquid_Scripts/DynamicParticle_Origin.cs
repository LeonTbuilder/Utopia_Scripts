...
public class Dynamic_Particle : MonoBehaviour {	
	...
	void OnCollisionEnter2D(Collision2D other){
		if(currentState==STATES.WATER && other.gameObject.tag=="Dynamic_Particle"){ 
			if(other.collider.GetComponent<Dynamic_Particle>().currentState==STATES.LAVA){
				SetState(STATES.GAS);
			}
		}
